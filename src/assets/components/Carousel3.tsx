...
import { getFirestore, doc, setDoc, Timestamp, DocumentReference } from "firebase/firestore";
...
const Carrusel: React.FC = () => {
  ...
  const handleScroll = (e: React.WheelEvent) => {
    ...
    setCurrentIndex((prev) =>
      e.deltaY > 0 ? Math.min(prev + 1, pesos.length - 1) : Math.max(prev - 1, 0)
    );
  };
  ...
  const savePeso = async () => {
    if (!user || !user.uid) { // Asegurándonos de que `user` y `user.uid` existen
    ...
    <div className="carrusel-container" onWheel={handleScroll}>
      ...
      <div className="carrusel">
        {pesos.map((peso, index) => (
          <div
            key={peso}
            className={`carrusel-item ${index === currentIndex ? "selected" : ""}`}
            style={{
